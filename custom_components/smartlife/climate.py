...
@dataclass
class SmartLifeClimateSensorDescriptionMixin:
...
@dataclass
class SmartLifeClimateEntityDescription(
    ClimateEntityDescription, SmartLifeClimateSensorDescriptionMixin
):
    """Describe an smartlife climate entity."""


CLIMATE_DESCRIPTIONS: dict[str, SmartLifeClimateEntityDescription] = {
    # Air conditioner
    # https://developer.tuya.com/en/docs/iot/categorykt?id=Kaiuz0z71ov2n
    "kt": SmartLifeClimateEntityDescription(
        key="kt",
    ...
    "rs": SmartLifeClimateEntityDescription(
        key="rs",
        switch_only_hvac_mode=HVACMode.HEAT,
    ),
    # Thermostat
    # https://developer.tuya.com/en/docs/iot/f?id=K9gf45ld5l0t9
    "wk": SmartLifeClimateEntityDescription(
        key="wk",
        switch_only_hvac_mode=HVACMode.HEAT_COOL,
    ),
    # Thermostatic Radiator Valve
    # Not documented
    "wkf": SmartLifeClimateEntityDescription(
        key="wkf",
        switch_only_hvac_mode=HVACMode.HEAT,
    ),
}
...
async def async_setup_entry(
    ...
    """Set up smartlife climate dynamically through smartlife discovery."""
    hass_data: HomeAssistantSmartLifeData = hass.data[DOMAIN][entry.entry_id]

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered smartlife climate."""
        entities: list[SmartLifeClimateEntity] = []
        for device_id in device_ids:
            device = hass_data.manager.device_map[device_id]
            if device and device.category in CLIMATE_DESCRIPTIONS:
                entities.append(
                    SmartLifeClimateEntity(
                        device,
                        hass_data.manager,
...
class SmartLifeClimateEntity(SmartLifeEntity, ClimateEntity):
    """smartlife Climate Device."""

    _current_humidity: IntegerTypeData | None = None
    _current_temperature: IntegerTypeData | None = None
    _hvac_to_smart_life: dict[str, str]
    _set_humidity: IntegerTypeData | None = None
    _set_temperature: IntegerTypeData | None = None
    entity_description: SmartLifeClimateEntityDescription

    def __init__(
        self,
        device: CustomerDevice,
        device_manager: Manager,
        description: SmartLifeClimateEntityDescription,
    ) -> None:
        ...
        self._hvac_to_smart_life = {}
        if enum_type := self.find_dpcode(
            DPCode.MODE, dptype=DPType.ENUM, prefer_function=True
        ):
            self._attr_hvac_modes = [HVACMode.OFF]
            unknown_hvac_modes: list[str] = []
            for smart_life_mode in enum_type.range:
                if smart_life_mode in SMART_LIFE_HVAC_TO_HA:
                    ha_mode = SMART_LIFE_HVAC_TO_HA[smart_life_mode]
                    self._hvac_to_smart_life[ha_mode] = smart_life_mode
                    self._attr_hvac_modes.append(ha_mode)
                else:
                    unknown_hvac_modes.append(smart_life_mode)

            if unknown_hvac_modes:  # smartlife modes are presets instead of hvac_modes
    ...
    def set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        if hvac_mode in self._hvac_to_smart_life:
            commands.append(
                {"code": DPCode.MODE, "value": self._hvac_to_smart_life[hvac_mode]}
            )
    ...
    def hvac_mode(self) -> HVACMode:
        ...
        ) is not None and mode in SMART_LIFE_HVAC_TO_HA:
            return SMART_LIFE_HVAC_TO_HA[mode]
    ...
    def preset_mode(self) -> str | None:
        ...
        mode = self.device.status.get(DPCode.MODE)
        if mode in SMART_LIFE_HVAC_TO_HA:
            return None
